...
export const ErdVisualizer: React.FC<ErdVisualizerProps> = ({ API_BASE, theme, selectedDbServer, selectedDatabase }) => {
  ...
  if (schema && schema.tables) {
    schema.tables.forEach((_, idx) => {
      const coords = getTableCoords(idx);
      if (coords.x + colWidth + 50 > maxX) maxX = coords.x + colWidth + 50;
      if (coords.y + 360 + 50 > maxY) maxY = coords.y + 360 + 50;
    });
  }
        ...
        <div style={{
          ...
          >
            {schema.relations.map((rel, idx) => {
              ...
              const y_from = y_A + 38 + fromColIdx * 31 + 15.5;
              const y_to = y_B + 38 + toColIdx * 31 + 15.5;
